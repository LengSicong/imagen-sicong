...
def sample(args):
    ...
    #     inpaint_init_image = 256. - inpaint_init_image
    # inpaint_init_image = torch.from_numpy(inpaint_init_image).unsqueeze(0).type(torch.float32).cuda()
    # inpaint_init_image = torch.transpose(inpaint_init_image, 1,3)
    # inpaint_init_image = torch.transpose(inpaint_init_image, 2,3)
    # inpaint_images = torch.randn(1, 3, 256, 256).cuda()
    # sample_images = imagen.sample(batch_size = 1,
    #                               texts=[inpaint_text],
    #                               inpaint_images = inpaint_init_image,
    ...
    # sample_images = imagen.sample(texts=[inpaint_text],
